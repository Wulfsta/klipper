...
LIS3DH_SCALE = FREEFALL_ACCEL * 3.906 / 16 # right shift for left justified registers
...
LIS2DW_TYPE = 'lis2dw'
LIS3DH_TYPE = 'lis3dh'
LIS_TYPES = [LIS2DW_TYPE, LIS3DH_TYPE]

SPI_SERIAL_TYPE = 'spi'
I2C_SERIAL_TYPE = 'i2c'
SERIAL_TYPES = [SPI_SERIAL_TYPE, I2C_SERIAL_TYPE]
...
class LIS2DW:
    def __init__(self, config, lis_type):
        ...
        if self.lis_type == LIS2DW_TYPE:
            self.axes_map = adxl345.read_axes_map(config, LIS2DW_SCALE, LIS2DW_SCALE, LIS2DW_SCALE)
            self.data_rate = 1600
        else:
            self.axes_map = adxl345.read_axes_map(config, LIS3DH_SCALE, LIS3DH_SCALE, LIS3DH_SCALE)
            self.data_rate = 1344
        # Check for spi or i2c
        self.serial_type = config.getchoice('serial_type', SERIAL_TYPES, default=SERIAL_TYPES[0])
        # Setup mcu sensor_lis2dw bulk query code
        if self.serial_type == SPI_SERIAL_TYPE:
            self.serial = bus.MCU_SPI_from_config(config, 3, default_speed=5000000)
        else:
            self.serial = bus.MCU_I2C_from_config(config, default_addr=LIS_I2C_ADDR, default_speed=400000)
        self.mcu = mcu = self.serial.get_mcu()
        self.oid = oid = mcu.create_oid()
        self.query_lis2dw_cmd = None
        mcu.add_config_cmd("config_lis2dw oid=%d serial_oid=%d serial_type=%d model=%d"
                           % (oid, self.serial.get_oid(), SERIAL_TYPES.index(self.serial_type), LIS_TYPES.index(self.lis_type)))
    ...
    def _build_config(self):
        cmdqueue = self.serial.get_command_queue()
    ...
    def read_reg(self, reg):
        if self.serial_type == SPI_SERIAL_TYPE:
            params = self.serial.spi_transfer([reg | REG_MOD_READ, 0x00])
            response = bytearray(params['response'])
            return response[1]
        params = self.serial.i2c_read([reg], 1)
        return bytearray(params['response'])[0]
    def set_reg(self, reg, val, minclock=0):
        if self.serial_type == SPI_SERIAL_TYPE:
            self.serial.spi_send([reg, val & 0xFF], minclock=minclock)
        else:
            self.serial.i2c_write([reg, val & 0xFF], minclock=minclock)
